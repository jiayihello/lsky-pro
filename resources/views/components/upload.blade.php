<div class="absolute inset-0 z-0 bg-fixed bg-cover bg-center animate-slide rightness-10" id="background-div" style="background-size: 100% 100%;"></div>
<script>
  var isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
  if (isMobile) {
    document.getElementById('background-div').style.backgroundImage = 'url(https://t.alcy.cc/lai)';
  } else {
    var videoUrl = 'https://v3-cold.douyinvod.com/13def3937ca6a974f28e7da7e06c7d45/6804e8b7/video/tos/cn/tos-cn-ve-15/ogwhcUymIIPANWDBCbQztA4ejVAfPBago9TzEs/?a=1128&ch=0&cr=0&dr=0&cd=0%7C0%7C0%7C0&cv=1&br=745&bt=745&cs=0&ds=3&ft=MgtpHQQqUuIf_oZmo0OW_QYaUqiXpjdvRVJEu1tNIbPD-Ipz&mime_type=video_mp4&qs=0&rc=OGg0ZDk4aDhkOjg0N2ZmM0BpMzVtZWk6ZmRvcjMzNGkzM0A2X2E2Yl41NjUxMy1gLS0tYSNwMTAxcjRvMGFgLS1kLWFzcw%3D%3D&btag=c0010e00088000&cquery=100y&dy_q=1745148573&feature_id=f0150a16a324336cda5d6dd0b69ed299&l=202504201929324B1158CD79EE893E99E3'; // Replace with the URL of your video.
    var videoElement = document.createElement('video');
    videoElement.src = videoUrl;
    videoElement.autoplay = true;
    videoElement.loop = true;
    videoElement.muted = true;
    videoElement.setAttribute('playsinline', ''); // For iOS autoplay.
    ...
    document.getElementById('background-div').appendChild(videoElement);
  }
</script>
...
<!-- 我去掉了"bg-white"类，并使用内联样式将背景颜色设置为半透明白色，
使用"rgba"函数。"rgba"函数的第四个值表示不透明度，其中1为完全不透明，0为完全透明。
在这种情况下，我使用了0.8，意味着背景将是80%不透明，呈现半透明效果。
您可以调整不透明度的值（从0.0到1.0）来实现所需的透明度水平。
我在上传区域的外部添加了一个新的<div>，它设置为绝对定位（position: absolute;），
并使用z-index属性来放置在上层（z-index: 1;）。这个<div>会将边框放在最上层，
而图片上传区域保持在下层（z-index: 2;）。这样，边框将显示在其他内容的上方，实现所需的效果。 -->
      <div class="mb-4 p-4 rounded-md shadow-custom" style="background-color: rgba(255, 255, 255, 0.3); position: relative;">
...
<!-- 以上为一段代码 -->
<!-- 我在 <style> 标签中定义了一个名为 .transparent-bg 的类，用于设置背景透明。
然后，我将这个类应用到 <div id="links-container"> 元素上，以实现透明背景效果。-->
<style>
    .transparent-bg {
...
<div id="links-container" class="hidden mb-4 p-4 rounded-md relative group shadow-custom transparent-bg">
    <div class="absolute top-2 right-2 flex">
                                                                                <!-- bg-gray-90    修改数值使按钮背景透明 -->
        <span id="copy-all" class="px-2 py-1 rounded-md text-xs text-gray-800 bg-gray-90 cursor-pointer hidden group-hover:block">复制全部</span>
        <span id="clear-all" class="ml-1 px-2 py-1 rounded-md text-xs text-gray-800 bg-gray-10 cursor-pointer hidden group-hover:block">清除</span>
                                                                                <!-- bg-gray-10    修改数值使按钮背景透明 -->
    </div>
    <div id="link-tabs" class="flex flex-nowrap overflow-scroll scrollbar-none text-sm">
                                                                                <!-- bg-gray-10    修改数值使按钮背景透明 -->
            <a href="javascript:void(0)" data-tab-name="url" class="hover:bg-gray-30 flex justify-center items-center px-8 py-2 border-b-2 border-indigo-500 active">URL</a>
            <a href="javascript:void(0)" data-tab-name="html" class="hover:bg-gray-30 flex justify-center items-center px-8 py-2 border-b-2 border-transparent">HTML</a>
            <a href="javascript:void(0)" data-tab-name="bbcode" class="hover:bg-gray-30 flex justify-center items-center px-8 py-2 border-b-2 border-transparent">BBCode</a>
            <a href="javascript:void(0)" data-tab-name="markdown" class="hover:bg-gray-30 flex justify-center items-center px-8 py-2 border-b-2 border-transparent">Markdown</a>
            <a href="javascript:void(0)" data-tab-name="markdown_with_link" class="hover:bg-gray-30 flex justify-center items-center px-8 py-2 border-b-2 border-transparent whitespace-nowrap">Markdown with link</a>
            <a href="javascript:void(0)" data-tab-name="thumbnail_url" class="hover:bg-gray-30 flex justify-center items-center px-8 py-2 border-b-2 border-transparent whitespace-nowrap">Thumbnail url</a>
                                                                                <!-- bg-gray-10    修改数值使按钮背景透明 -->
        </div>
...
<!-- transparent-bg 类的 <div> 元素具有透明背景。-->
<script type="text/html" id="image-preview-tpl">
    <div data-id="__id__" class="w-full flex items-center p-2 mb-2 rounded-md relative transparent-bg bg-gray-00 overflow-hidden">
        <div class="absolute inset-0">
            <div class="w-[0%] h-full bg-gray-200 opacity-0 upload-progress"></div>
        </div>
        ...
        <div class="absolute right-2 flex space-x-2">
            <a href="javascript:void(0)" data-operate="remove" class="flex justify-center items-center block shadow-sm w-10 h-10 rounded-full text-gray-600 bg-gray-10 hover:bg-gray-200 aspect-w-1 aspect-h-1"><i class="fas fa-times"></i></a>
            <a href="javascript:void(0)" data-operate="upload" class="flex justify-center items-center block shadow-sm w-10 h-10 rounded-full text-gray-600 bg-gray-10 hover:bg-gray-200 aspect-w-1 aspect-h-1"><i class="fas fa-upload"></i></a>
        </div>
    </div>
</script>
<!-- transparent-bg 类的 <div> 元素具有透明背景。-->
            ...
            progress: (e, data) => {
                ...
                if (response.status) {
                    ...
                    for (let key in links) {
                        $('#links [data-tab="' + key + '"]').append('<p class="whitespace-nowrap select-all mt-1 bg-gray-0 hover:bg-gray-0 text-gray-20 rounded px-2 py-1 cursor-pointer overflow-scroll scrollbar-none">' + links[key].toString() + '</p>')
                    }
                    $links.show();
